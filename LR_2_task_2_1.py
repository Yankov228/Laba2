...
classifier = OneVsOneClassifier(SVC(kernel='poly', degree=3))
...
# Проведення оцінки інших методів
metrics = ['accuracy', 'precision_weighted', 'recall_weighted']

for metric in metrics:
    scores = cross_val_score(classifier, X, Y, cv=5, scoring=metric)
    print(f"{metric}: {100*scores.mean():.2f}%")